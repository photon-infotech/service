...
public final class ServerUtil {
	...
	public static ArtifactGroup getArtifactinfo(InputStream inputJarStream,
			String fileExt) throws PhrescoException {
		File jarFile = writeFileFromStream(inputJarStream, null, fileExt,
				"validate");
	...
	public static InputStream getArtifactPomStream(File artifactFile)
			throws PhrescoException {
		String pomFile = null;
		if (getFileExtension(artifactFile.getName()).equals(
				ServerConstants.JAR_FILE)) {
			try {
	 ...
	 * @throws PhrescoException
	 */
	public static File writeFileFromStream(InputStream inputStream,
			String path, String packaging, String fileName)
			throws PhrescoException {
	...
	public static File createPomFile(ArtifactGroup info)
		...
		try {
			...
			Element rootElement = newDoc
					.createElement(ServerConstants.POM_PROJECT);
			newDoc.appendChild(rootElement);

			Element modelVersion = newDoc
					.createElement(ServerConstants.POM_MODELVERSION);
			...
			Element artifactId = newDoc
					.createElement(ServerConstants.POM_ARTIFACTID);
			...
			Element packaging = newDoc
					.createElement(ServerConstants.POM_PACKAGING);
			packaging.setTextContent(info.getPackaging());
			rootElement.appendChild(packaging);

			Element description = newDoc
					.createElement(ServerConstants.POM_DESC);
	 ...
	 * @return
	 */
	public static String createContentURL(String groupId, String artifactId,
	 ...
	 * @throws PhrescoException
	 */
	public static String encodeUsingHash(String userName, String password)
			throws PhrescoException {
		...
		try {
			...
			for (int i = 0; i < messageDigests.length; i++) {
				stringBuffer.append(Integer.toHexString(MAGICNUMBER.HEXADECIMAL
						& messageDigests[i]));
			}
	...
	public static File getArtifactPomFile(File artifactFile)
			throws PhrescoException {
	...
	public static boolean convertByteArrayToFile(File file, byte[] bytes)
			throws PhrescoException {
	...
	public static String getSize(String sKey) throws PhrescoException {
		...
		String sValue="";
		try {
			InputStream is = ServerUtil.class.getClassLoader()
